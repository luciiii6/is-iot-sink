...
sudo echo "function sink-setup-env {cd /home/pi/is-iot-sink && source env/bin/activate && set -o allexport; source .env; set +o allexport}" | sudo tee -a /usr/local/bin/startup.sh
sudo echo "function sink-run {sink-setup-env && python is_iot_sink/main.py}" | sudo tee -a /usr/local/bin/startup.sh
